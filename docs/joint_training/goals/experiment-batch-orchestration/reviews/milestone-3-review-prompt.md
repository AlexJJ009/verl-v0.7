...
- Candidate commit: `066c34da0d41254db22e9db542b1b83c7a34f460`
...
Adversarially review monitor and Stage123 migration from committed main and recipe states. Verify core owns all phase/item transitions; Stage123 queue shell has no status/deadline/cleanup/failure/phase-loop authority; batch no-retry is enforced; monitor is one read-only persisted-event implementation with idempotent cursor; phase failure skips remaining phases and falls to next item. Audit deletion-budget and authority-inventory against actual diff. Do not implement.
