...
control=pd.concat([stage2.assign(control_local_step=stage2.local_step),stage3.assign(control_local_step=20+stage3.local_step)],ignore_index=True)
...
step_rows=[]
for phase,df,step_col in [('stage2',stage2,'local_step'),('stage3',stage3,'local_step'),('control',control,'control_local_step')]:
 for step,g in df.groupby(step_col):
  ...
  row={'phase':phase,'local_step':int(step),'effective_step':int(g.effective_step.iloc[0]),'rows':len(g),
       'mean_level':g.level.mean(),'level4plus_rate':(g.level>=4).mean(),'level5_rate':(g.level>=5).mean(),
       'mean_question_chars':g.question_chars.mean(),'mean_solution_chars':g.solution_chars.mean(),
       'mean_question_tokens_proxy':g.question_tokens_proxy.mean(),'mean_solution_tokens_proxy':g.solution_tokens_proxy.mean(),
       'subject_entropy':entropy}
  for subject,p in counts.items(): row['subject_'+re.sub(r'\W+','_',subject.lower()).strip('_')+'_rate']=p
...
for phase,df,step_col,ranges in [
 ...
 ('control',control,'control_local_step',[(1,20),(21,30),(31,35),(36,40),(41,45),(46,50),(51,55),(56,60)])]:
 for a,b in ranges:
  g=df[df[step_col].between(a,b)]
  counts=g.subject.value_counts(normalize=True)
  row={'phase':phase,'window':f'{a}-{b}','effective_window':f'{int(g.effective_step.min())}-{int(g.effective_step.max())}',
       'rows':len(g),'mean_level':g.level.mean(),'level4plus_rate':(g.level>=4).mean(),'level5_rate':(g.level>=5).mean(),
       'mean_question_chars':g.question_chars.mean(),'mean_solution_chars':g.solution_chars.mean(),
       'mean_question_tokens_proxy':g.question_tokens_proxy.mean(),'mean_solution_tokens_proxy':g.solution_tokens_proxy.mean(),
       'subject_entropy':-sum(p*math.log(p+1e-12) for p in counts)}
  for s,p in counts.items(): row['subject_'+re.sub(r'\W+','_',s.lower()).strip('_')+'_rate']=p
  windows.append(row)
...
for phase,df in [('stage2',stage2),('stage3',stage3)]:
 x=df.groupby(['subject','level'],dropna=False).size().reset_index(name='rows'); x.insert(0,'phase',phase); parts.append(x)
