...
offsets = {
    "b0-stage1": 0,
    "b01-stage1": 0,
    "b0-stage1-control": 40,
    "b01-stage1-control": 40,
    "b0-stage2-nokl": 40,
    "b0-stage2-m2kl": 40,
    "b01-stage2-nokl": 40,
    "b01-stage2-m2kl": 40,
    "b0-stage3-nokl-model1": 60,
    "b0-stage3-nokl-model2": 60,
    "b0-stage3-m2kl-model1": 60,
    "b0-stage3-m2kl-model2": 60,
    "b01-stage3-nokl-model1": 60,
    "b01-stage3-nokl-model2": 60,
    "b01-stage3-m2kl-model1": 60,
    "b01-stage3-m2kl-model2": 60,
}
...
agg = train.groupby(["id", "block_end", "effective_block_end"], as_index=False).agg(
    {
        "wdl_sft/correct_ratio": "mean",
        "critic/rewards/mean": "mean",
        "response_length/mean": "mean",
        "response_length/clip_ratio": "mean",
        "actor/grad_norm": "mean",
        "actor/pg_loss": "mean",
        "actor/wdl_sft_loss_total": "mean",
    }
)
...
data5 = stage3_data.groupby("block_end", as_index=False).agg(
    {
        "mean_level": "mean",
        "level4plus_rate": "mean",
        "level5_rate": "mean",
        "mean_question_chars": "mean",
        "mean_solution_chars": "mean",
        "subject_entropy": "mean",
    }
)
model2 = focus[focus.id.str.contains("stage3") & focus.id.str.contains("model2")]
perblock = model2.groupby("block_end", as_index=False).agg(
    {"delta_mean3": "mean", "wdl_sft/correct_ratio": "mean", "response_length/mean": "mean"}
)
...
for x in [
    ...
    for y in ["delta_mean3", "wdl_sft/correct_ratio", "response_length/mean"]:
        pr = pearsonr(data_join[x], data_join[y])
        sr = spearmanr(data_join[x], data_join[y])
        cor.append(
            {
                "x": x,
                "y": y,
...
colors = {
    "b0-stage1-control": "#111827",
    "b01-stage1-control": "#6b7280",
    "b0-stage3-nokl-model2": "#2563eb",
    "b0-stage3-m2kl-model2": "#dc2626",
    "b01-stage3-nokl-model2": "#16a34a",
    "b01-stage3-m2kl-model2": "#9333ea",
}
...
for run, c in colors.items():
    ...
    axes[1, 1].plot(
        p.effective_block_end, p["response_length/clip_ratio"] * 100, marker="o", label=labels[run], color=c
    )
for ax in axes.ravel():
    ax.axvspan(75, 85, color="#f59e0b", alpha=0.12, label="observed transition 75-85")
    ax.axvline(80, color="#9ca3af", ls=":", lw=1)
    ax.grid(alpha=0.25)
    ax.set_xlim(40, 100)
axes[0, 0].set_title("Math-7 macro mean@3")
axes[0, 0].set_ylabel("%")
axes[0, 1].set_title("Training correct ratio (5-step mean)")
axes[0, 1].set_ylabel("%")
axes[1, 0].set_title("Mean rollout response length (5-step mean)")
axes[1, 0].set_ylabel("tokens")
axes[1, 1].set_title("Rollout truncation/clip ratio (5-step mean)")
axes[1, 1].set_ylabel("%")
for ax in axes[1]:
    ax.set_xlabel("Effective step")
handles, lab = axes[0, 0].get_legend_handles_labels()
fig.legend(handles, lab, loc="lower center", ncol=3, fontsize=9)
fig.suptitle("Math Stage123: validation jump and training dynamics", fontsize=16)
fig.tight_layout(rect=(0, 0.08, 1, 0.96))
fig.savefig(ROOT / "math_stage123_training_dynamics.png", dpi=180)
plt.close(fig)
...
for ax in axes.ravel():
    ax.axvspan(16, 25, color="#f59e0b", alpha=0.15)
    ax.grid(alpha=0.25)
    ax.set_xlim(1, 40)
axes[0, 0].plot(d.local_step, d.mean_level, marker="o")
axes[0, 0].set_title("Mean Hendrycks MATH level per batch")
axes[0, 1].plot(d.local_step, d.level4plus_rate * 100, marker="o", label="Level >=4")
axes[0, 1].plot(d.local_step, d.level5_rate * 100, marker="o", label="Level 5")
axes[0, 1].legend()
axes[0, 1].set_title("Difficulty mix per 64-row batch")
axes[0, 1].set_ylabel("%")
axes[1, 0].plot(d.local_step, d.mean_question_chars, marker="o", label="question chars")
axes[1, 0].plot(d.local_step, d.mean_solution_chars, marker="o", label="solution chars")
axes[1, 0].legend()
axes[1, 0].set_title("Reference text length")
subject_cols = [c for c in d.columns if c.startswith("subject_") and c.endswith("_rate")]
for c in subject_cols:
    axes[1, 1].plot(d.local_step, d[c] * 100, label=c[len("subject_") : -len("_rate")].replace("_", " "), alpha=0.8)
axes[1, 1].legend(fontsize=8, ncol=2)
axes[1, 1].set_title("Subject composition per batch")
axes[1, 1].set_ylabel("%")
for ax in axes[1]:
    ax.set_xlabel("Stage3 local step (fixed data order)")
fig.suptitle("Stage3 fixed-order training data windows; shaded = local steps 16-25", fontsize=16)
fig.tight_layout(rect=(0, 0, 1, 0.96))
fig.savefig(ROOT / "math_stage123_data_window_analysis.png", dpi=180)
plt.close(fig)
