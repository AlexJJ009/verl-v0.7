...
def wandb_dir(log):
    ...
    if not matches:
        raise RuntimeError(f"no wandb path {log}")
...
def rows(path):
    ...
    while True:
        ...
        for x in pb.history.item:
            ...
            try:
                item[key] = json.loads(x.value_json)
            except (TypeError, json.JSONDecodeError):
                item[key] = x.value_json
