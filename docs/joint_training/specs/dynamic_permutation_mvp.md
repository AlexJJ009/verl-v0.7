...
The resource-threshold smoke uses the production defaults `row_chunk_size=8` and `audit_rows=4`; invariant auditing remains enabled on every active step. This keeps the measured overhead representative of the admitted MVP configuration rather than an artificial 8x row-chunk and audit stress setting.
