...
Archive location update: moved from `plans/active/` to `plans/completed/` on
2026-05-18 after the 3A failure analysis was recorded. This is no longer an
...
- Detailed failure analysis is recorded in `docs/joint_training/plans/completed/dual_submodel_rollout_wdl_sft_3a_failure_analysis.md`.
...
- `docs/joint_training/plans/completed/dual_submodel_rollout_wdl_sft_status.md` (archived from `plans/active/`)
- `docs/joint_training/plans/completed/dual_submodel_rollout_wdl_sft.md` (archived from `plans/active/`)
...
- `docs/joint_training/plans/completed/dual_submodel_rollout_wdl_sft_goal.md` (archived from `plans/active/`)
...
- Historical branch note: before the implementation branch work, these files
  were dirty or untracked and intentionally not staged there:
  `.codex/config.toml`, `docs/joint_training/plans/active/ablation_single_model.md`,
  `docs/joint_training/plans/active/wdl_sft_is.md`,
  `.claude/skills/experiment-registry`,
  `docs/joint_training/plans/active/dual_submodel_rollout_wdl_sft_goal.md`,
  and `recipe/on_policy_wdl_sft/EXPERIMENT_INDEX.md`.
...
- Historical next action at branch handoff was to stop the current 3A training
  process, preserve logs/metrics/validation dumps, and switch back to the
  previous branch. The documentation-side action is complete: this plan is
  archived as a negative result.
