...
| `qwen3_1p7b_wdl_mechanism_program.md` | DynPerm rho=0/0.25/0.5 P60 complete; rho=1 confirmation arms running; offline eval pending | The completed partial matrix shows that preserving weak entropy, target probability and value multiset is insufficient: changing non-target value-to-token assignment severely degrades training, especially with joint Model1 updates. This establishes dependence on real assignment or the cross-model geometry that the intervention also changes; Align/Random/Anti and scalar-geometry controls are required before attributing the effect to token semantics. |
