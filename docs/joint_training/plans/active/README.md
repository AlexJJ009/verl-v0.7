...
| `qwen3_1p7b_wdl_mechanism_program.md` | Active theory-derived mechanism design; DynPerm P60 two-arm launcher in review, not launched | Unifies reward-projected hard self-distillation with an exact product-of-experts/Chernoff decomposition, then defines Math-first fixed-M1, weak-entropy-preserving permutation, affinity-direction controls, a no-online-WM1 target-margin controller, and reverse rollout/loss experiments. Dynamic Permutation core code is merged with final focused CPU evidence and Job 146 8xL40S FSDP engineering smoke `PASS`; the prepared P60 queue crosses Standard C and fixed-Model1 through only `DYNPERM_ENABLED` / `DYNPERM_RHO`, remains `launch_allowed=false`, and no formal DynPerm run has started. |
