...
| `qwen3_1p7b_wdl_mechanism_program.md` | Active theory-derived mechanism design; DynPerm formal scripts prepared, not launched | Unifies reward-projected hard self-distillation with an exact product-of-experts/Chernoff decomposition, then defines Math-first fixed-M1, weak-entropy-preserving permutation, affinity-direction controls, a no-online-WM1 target-margin controller, and reverse rollout/loss experiments. Dynamic Permutation core code is merged into the formal training branch with final focused CPU evidence and Job 146 8xL40S FSDP engineering smoke `PASS`; prepared P20/P30/P60 wrappers remain `launch_allowed=false`, and no formal DynPerm run has started. |
