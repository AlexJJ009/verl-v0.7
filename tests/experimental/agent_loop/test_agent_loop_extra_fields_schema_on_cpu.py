...
class _FakeBatchEncodingTokenizer(_FakeTokenizer):
    def apply_chat_template(
        ...
        **kwargs,
        ...
        input_ids = [7] + [100 + idx for idx in range(len(messages))]
...
def test_agent_loops_normalize_batch_encoding_chat_templates_to_token_ids():
    async def _run():
        ...
        assert single_turn.system_prompt == [7]
        assert await single_turn.apply_chat_template(raw_prompt, remove_system_prompt=True) == [100, 200]

        out_a = await single_turn.run(sampling_params={}, raw_prompt=raw_prompt)
        out_b = await partial_single_turn.run(sampling_params={}, raw_prompt=raw_prompt, param_version=0)

        assert out_a.prompt_ids == [7, 100, 200]
        assert out_b.prompt_ids == [7, 100, 200]
