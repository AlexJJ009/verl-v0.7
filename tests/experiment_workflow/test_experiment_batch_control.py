...
def test_stale_and_replayed_controls_do_not_mutate_state(tmp_path: Path) -> None:
    ...
    executor = tool.BatchExecutor(
        manifest, tmp_path / "state", support_module.FakeAdapter([0]), support_module.FakeClock()
    )
    manifest.operator_control_path.write_text(
        json.dumps(support_module.control(tool, manifest, 1, 0, "pause_after_current")) + "\n"
    )
    result = executor.run()
...
def test_pause_then_continue_uses_persisted_control_cursor(tmp_path: Path) -> None:
    ...
    manifest.operator_control_path.write_text(
        __import__("json").dumps(support_module.control(tool, manifest, 1, 0, "pause_after_current")) + "\n"
    )
    assert first.run()["status"] == "paused_after_current"
    with manifest.operator_control_path.open("a") as handle:
        handle.write(
            __import__("json").dumps(support_module.control(tool, manifest, 2, 1, "continue_remaining")) + "\n"
        )
