...
def test_no_agent_timer_or_tuning_authority_in_batch_paths() -> None:
    ...
    queue = (ROOT / "recipe/on_policy_wdl_sft/code_task/run_code_task_qwen3_1p7b_stage123_queue_impl.sh").read_text().lower()
...
def test_deletion_budget_and_inventory_are_consistent() -> None:
    budget = json.loads((ROOT / "docs/joint_training/goals/experiment-batch-orchestration/deletion-budget.json").read_text())
    inventory = json.loads((ROOT / "docs/joint_training/goals/experiment-batch-orchestration/authority-inventory.json").read_text())
