...
def test_dry_run_is_scratch_only_and_labeled(tmp_path: Path):
    ...
    result = run({"ALLOW_CODE_PREFLIGHT_GPU_SMOKE": "1", "PREFLIGHT_GPU_SMOKE_DRY_RUN": "1", "PREFLIGHT_MANIFEST": str(manifest), "PREFLIGHT_OUTPUT_ROOT": str(scratch)})
    assert result.returncode == 0
...
def test_non_scratch_output_is_rejected(tmp_path: Path):
    manifest = tmp_path / "manifest.json"; manifest.write_text("{}")
    result = run({"ALLOW_CODE_PREFLIGHT_GPU_SMOKE": "1", "PREFLIGHT_GPU_SMOKE_DRY_RUN": "1", "PREFLIGHT_MANIFEST": str(manifest), "PREFLIGHT_OUTPUT_ROOT": "/data-1/checkpoints/not-allowed"})
    assert result.returncode != 0
