...
def test_live_inventory_is_explicit_and_stage123_is_manifest_native(tmp_path: Path):
    ...
    native = {"code_task/run_code_task_qwen3_1p7b_stage123_queue.sh", "code_task/monitor_code_task_qwen3_1p7b_stage123_notify.sh"}
...
def test_inventory_report_contains_no_silent_classification(tmp_path: Path):
    ...
    result = subprocess.run(["python3", str(ROOT / "scripts/experiment_manifest.py"), "inventory", "--root", str(ROOT / "recipe/on_policy_wdl_sft"), "--output", str(output)], capture_output=True, text=True)
    assert result.returncode == 0
    report = json.loads(output.read_text())
    assert report["entries"]
    assert all(item["classification"] in {"manifest-native", "legacy-traceable", "legacy-unresolved"} and item["evidence_paths"] for item in report["entries"])
