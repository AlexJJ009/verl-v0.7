...
def tool():
    p = ROOT / "scripts/experiment_notification_policy.py"
    s = importlib.util.spec_from_file_location("notify", p)
...
def test_only_three_positive_events_and_non_events():
    ...
    for state in (
        ...
        assert m.event_for(state) is None
...
def test_dedup_redaction_paths_and_delivery_failure(tmp_path: Path):
    ...
    sender = tmp_path / "fail.sh"
    sender.write_text("#!/bin/sh\nexit 7\n")
    sender.chmod(0o755)
    state = {
        "run_id": "r",
        "training_step": 1,
        "background": "TOKEN=secret",
        "evidence": "e",
        "cost": "c",
        "recommendation": "r",
        "local_paths": "/data-2/evidence",
    }
    first = m.process(state, ledger, [str(sender)])
    second = m.process(state, ledger, [str(sender)])
    assert first == {"event": "run_started", "sent": False, "reason": "delivery_failed"}
    assert second["reason"] == "duplicate"
    record = json.loads(ledger.read_text())
    assert "secret" not in record["message"]
    assert "/data-2/evidence" in record["message"]
