...
def test_monitor_maps_only_persisted_core_events() -> None:
    ...
    base = {"schema_version": 1, "run_id": "stage123-primary-queue", "attempt": 1, "transition": {"from": "pending", "to": "running", "at": 1.0}, "failure": None, "cleanup": None}
    expected = {"pending": None, "running": "run_started", "succeeded": None}
    for status, event_name in expected.items():
        state = monitor.notification_state_from_event({**base, "status": status}, ["frac25-stage2", "frac25-stage3"], Path("/state"))
    ...
    for status in ("failed", "deadline_exceeded", "cleanup_failed"):
        event = {**base, "status": status, "failure": {"code": status, "message": "failed", "context": {}}, "cleanup": {"resources_released": status != "cleanup_failed"}}
...
def test_monitor_replays_core_events_without_external_runtime_probes(tmp_path: Path) -> None:
    ...
    events = [
        {"schema_version": 1, "run_id": "stage123-primary-queue", "status": "running", "attempt": 1, "transition": {"from": "pending", "to": "running", "at": 1}, "failure": None, "cleanup": None},
        {"schema_version": 1, "run_id": "stage123-primary-queue", "status": "failed", "attempt": 1, "transition": {"from": "running", "to": "failed", "at": 2}, "failure": {"code": "child_exit", "message": "failed", "context": {}}, "cleanup": {"resources_released": True}},
        {"schema_version": 1, "run_id": "stage123-primary-queue", "status": "succeeded", "attempt": 2, "transition": {"from": "running", "to": "succeeded", "at": 3}, "failure": None, "cleanup": {"resources_released": True}},
    ]
    (state_root / "events.jsonl").write_text("".join(json.dumps(event) + "\n" for event in events))
    (state_root / "stage123-primary-queue.json").write_text(json.dumps({"schema_version": 1, "run_id": "stage123-primary-queue", "status": "succeeded", "attempt": 2}))
    ledger = tmp_path / "notifications.jsonl"
    result = subprocess.run([sys.executable, str(MONITOR), "--manifest", str(manifest), "--state-root", str(state_root), "--ledger", str(ledger), "--policy", str(ROOT / "scripts/experiment_notification_policy.py"), "--once"], text=True, capture_output=True, check=False)
