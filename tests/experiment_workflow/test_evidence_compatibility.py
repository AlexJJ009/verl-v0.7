...
def test_only_three_result_classes_can_authorize_current_execution():
    ...
    bindings = {
        ...
        "resource_profile_sha256": calibration["resource_profile_sha256"],
        "implementation_tree_sha256": calibration["implementation_tree_sha256"],
        "evidence_commit": calibration["evidence_commit"],
        "run_ids": calibration["workload_identity"]["run_ids"],
        "authorization_identity": calibration["authorization_identity"],
    }
...
def test_stale_or_malformed_result_files_fail_closed(tmp_path: Path):
    ...
    legacy.write_text(json.dumps({"receipt_type": "code_task_operational_calibration_deployability"}))
    assert tool.load_and_validate(legacy).code == "legacy_evidence"


def test_incomplete_calibration_result_never_authorizes():
    tool = module()
    decision = tool.validate_result(
        {"schema_version": 1, "result_type": "calibration_result", "manifest_sha256": "a" * 64, "decision": "passed"}
    )
