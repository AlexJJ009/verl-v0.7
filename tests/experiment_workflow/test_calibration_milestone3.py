...
def test_boundary_manifest_literal_contract():
    ...
    assert value["superproject_tree_roots"] == ["config/experiment_execution", "scripts", "verl"] and value[
        "submodules"
...
def test_probe_renderer_rejects_training_and_non_scratch(tmp_path):
    base = [
        sys.executable,
        str(ROOT / "scripts/render_calibration_probe_command.py"),
        "--manifest",
        str(ROOT / "recipe/on_policy_wdl_sft/experiment_manifest/stage123.yaml"),
        "--resource-profile",
        str(ROOT / "recipe/on_policy_wdl_sft/code_task/qwen3_1p7b_stage123_resource_profile.sh"),
        "--phases",
    ...
    bad = subprocess.run(
        base + ["--training-steps", "1", "--scratch-root", "/data-1/tmp/verl_agent_scratch/calibration"],
        capture_output=True,
        text=True,
    )
    assert bad.returncode == 1 and "training_steps" in bad.stderr
    bad = subprocess.run(
        base + ["--training-steps", "0", "--scratch-root", str(tmp_path)], capture_output=True, text=True
    )
    assert bad.returncode == 1 and "scratch_root" in bad.stderr
...
def test_probe_renderer_is_zero_step_json():
    ...
    assert (
        command[1].endswith("run_calibration_probe_zero_step.py")
        and "--training-steps" in command
        and command[command.index("--training-steps") + 1] == "0"
        and command[command.index("--optimizer-enabled") + 1] == "false"
    )
    history = Path(command[command.index("--prediction-history-result") + 1])
    assert history == ROOT / "docs/joint_training/goals/calibration-qualification/calibration_result.json"
...
def test_prediction_fails_closed_and_qualifies():
    m = load("calibration_prediction", ROOT / "scripts/calibration_prediction.py")
    policy = json.loads((ROOT / "config/experiment_execution/calibration_policy_v1.json").read_text())
    assert m.qualify([1, 2], 10, 10, policy).code == "insufficient_history"
    assert m.qualify([1, 2, 3], 10, 13, policy).code == "prediction_exceeded"
    assert m.qualify([1, 2, 3], 10, 12, policy).qualified
...
def test_probe_prediction_comparison_is_real_and_fails_closed(tmp_path):
    ...
    for metric in m.PREDICTION_METRICS:
        comparisons.append(
            {
                "metric": metric,
                "history": [8.0, 9.0, 10.0],
                "history_count": 3,
                "predicted_bound": 10.0,
                "observed_maximum": 9.0,
                "decision": {"qualified": True, "code": "qualified"},
            }
        )
    source = {
        "decision": "passed",
    ...
    repetition = {
        ...
        "metrics": {"validation_elapsed_seconds": 9.0},
        "resources": {"peak_rss_gib": 12.0, "gpu_wait_fraction": 0.8},
    }
    ...
    source["prediction_comparison"]["policy_sha256"] = "b" * 64
    source_path.write_text(json.dumps(source))
    try:
...
def test_result_schema_requires_complete_structured_result():
    m = load("calibration_result", ROOT / "scripts/calibration_result.py")
    schema = json.loads((ROOT / "config/experiment_execution/calibration_result_schema_v1.json").read_text())
    assert (
        m.validate({"schema_version": 1, "result_type": "calibration_result", "decision": "passed"}, schema)[
            "failures"
        ][0]["code"]
        == "result_fields"
    )
    ...
    comparison = {
        ...
        "predicted_bound": 3,
        "observed_maximum": 3,
        "decision": {"qualified": True, "code": "qualified", "context": {"ratio": 1.0}},
    }
    comparisons = [
        {**comparison, "metric": metric}
        for metric in ("validation_elapsed_seconds", "phase_elapsed_seconds", "peak_rss_gib", "gpu_wait_fraction")
    ]
    value = {
        "schema_version": 1,
        ...
        "workload_identity": {"sha256": "e" * 64},
        "policy_id": "stage123-calibration-policy-v1",
        "policy_sha256": m.policy_sha256(policy),
        "authorization_identity": {"id": "auth"},
        "started_at": "2026-01-01T00:00:00Z",
        "completed_at": "2026-01-01T00:01:00Z",
        "phase_evidence": [
            {"phase": "stage1", "status": "passed"},
            {"phase": "stage2", "status": "passed"},
            {"phase": "stage3", "status": "passed"},
        ],
        "prediction_comparison": {
            "qualified": True,
            "policy_id": "stage123-calibration-policy-v1",
    ...
    assert m.validate(value, schema)["ok"]
    value["cleanup"]["resources_released"] = False
    assert m.validate(value, schema)["failures"][0]["code"] == "cleanup"
    value["cleanup"]["resources_released"] = True
    value["phase_evidence"] = value["phase_evidence"][1:]
    assert m.validate(value, schema)["ok"]
    value["phase_evidence"].reverse()
    assert m.validate(value, schema)["failures"][0]["code"] == "phase_evidence"
...
def test_zero_step_driver_runs_exact_phase_matrix_and_stops_on_failure(tmp_path, monkeypatch):
    ...
    def fake_run(rendered, phase, repetition, root, splits, timeout):
        calls.append((phase, repetition, timeout))
        return {"status": "failed" if phase == "stage3" and repetition == 2 else "passed"}
    ...
    for phase in ("stage1", "stage2", "stage3"):
        for repetition in range(1, 4):
            value = m.run_repetition(rendered, phase, repetition, tmp_path, {}, 1800)
...
def test_stage3_proxy_is_explicit_and_hash_bound():
    ...
    workload = rendered["calibration_workloads"]["stage3"]
    proxy = workload["calibration_proxy"]
...
def test_file_logger_metrics_join_quoted_fragments(tmp_path):
    ...
    for index, key in enumerate(ordered):
        lines.extend(
            [
                f"\x1b[36m(TaskRunner pid=1)\x1b[0m  {repr(repr(key) + ': ')}",
                f"\x1b[36m(TaskRunner pid=1)\x1b[0m  {repr(str(index + 1.0) + ', ')}",
            ]
        )
    (logs / "CALIBRATION-STAGE2.log").write_text("\n".join(lines))
