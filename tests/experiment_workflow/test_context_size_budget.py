...
def test_oversized_startup_document_fails(tmp_path: Path):
    path = tmp_path / "AGENTS.md"; path.write_text("x\n" * 401)
    assert not within_budget(path)


def test_exception_contract_has_required_fields_and_caps():
    exception = {"owner": "owner", "reason": "reason", "plan": "docs/plan.md", "reviewer": "reviewer", "expiry": "2026-07-01", "max_lines": 400, "max_bytes": 131072}
