...
class RayPPOTrainer:
    ...
    def _dump_generations(
        self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path, data_sources=None, sample_uids=None
    ):
        ...
        if sample_uids is not None and len(sample_uids) == n:
            base_data["uid"] = [_normalize_validation_generation_value(uid) for uid in sample_uids]
    ...
    def init_workers(self):
        ...
        if OmegaConf.select(self.config.trainer, "ray_master_port_range") is not None:
            wg_kwargs["master_port_range"] = OmegaConf.to_container(
                self.config.trainer.ray_master_port_range, resolve=True
            )
    ...
    def _compute_old_log_prob(self, batch: DataProto):
        ...
        if self.use_legacy_worker_impl == "disable":
            ...
            if calculate_entropy:
                ...
                tensor_dict["entropys"] = entropy.float()
            old_log_prob = tu.get_tensordict(tensor_dict)
            old_log_prob = DataProto.from_tensordict(old_log_prob)
        else:
            previous_flag = batch.meta_info.get("calculate_entropy", None)
            ...
            try:
                ...
                if previous_flag is None:
                    ...
                    batch.meta_info["calculate_entropy"] = previous_flag
            old_log_prob_mfu = 0
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # The group-advantage variant is deliberately excluded from this path.
