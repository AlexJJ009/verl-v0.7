...
def main() -> int:
    ...
    sent = {json.loads(line)["key"] for line in args.ledger.read_text().splitlines() if line.strip()} if args.ledger.exists() else set()
    offset = 0
    missing_polls = 0
    while True:
