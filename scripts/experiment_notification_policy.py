...
def event_for(state: dict) -> str | None:
    ...
    if state.get("authority_type") == "experiment_execution_core_event_v1":
        ...
        if status == "running":
            return "run_started"
        if (
            status in {"failed", "deadline_exceeded", "cleanup_failed"}
            and isinstance(state.get("failure"), dict)
            and isinstance(state.get("cleanup"), dict)
        ):
            return "run_failed"
        return None
    if state.get("terminal_failure") and state.get("cleanup_evidence"):
        return "run_failed"
    if int(state.get("training_step", 0)) >= 1 or state.get("complete_validation_metrics") is True:
        return "run_started"
    return None
...
def process(state: dict, ledger_path: Path, sender: list[str] | None) -> dict:
    ...
    if event not in ALLOWED:
        return {"event": None, "sent": False, "reason": "non_event"}
    ...
    if any(item.get("dedup_key") == key for item in ledger):
        return {"event": event, "sent": False, "reason": "duplicate"}
    message = redact(
        "\n".join(
            str(state.get(field, "")) for field in ("background", "evidence", "cost", "recommendation", "local_paths")
        )
    )
    ...
    return {
        "event": event,
        "sent": delivery["returncode"] == 0,
        "reason": "delivered" if delivery["returncode"] == 0 else "delivery_failed",
    }
...
def main() -> int:
    ...
    result = process(json.loads(args.state.read_text()), args.ledger, args.sender)
