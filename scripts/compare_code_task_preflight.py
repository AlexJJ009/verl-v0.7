...
def compare(before: dict, after: dict) -> dict:
    ...
    for old, new in zip(before["phases"], after["phases"], strict=True):
        ...
        phase_results.append({"phase": old["phase"], "throughput_gain": round(throughput_gain, 6), "timeout_rate_delta": round(timeout_delta, 6), "peak_rss_change": round(rss_gain, 6), "hard_wall_pass": hard_wall, "optimized": optimized})
    return {"ok": all(item["optimized"] for item in phase_results), "decision": "optimized" if all(item["optimized"] for item in phase_results) else "neutral_or_regressed", "phases": phase_results}
