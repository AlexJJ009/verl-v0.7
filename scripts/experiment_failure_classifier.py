...
RULES = (
    ...
    (
        "config_path_failure",
        re.compile(
            r"No such file or directory|FileNotFoundError|path.*(?:missing|mismatch)|config.*(?:invalid|mismatch)", re.I
        ),
    ),
    ...
    (
        "early_queue_exit",
        re.compile(r"queue.*(?:exited|stopped).*(?:early|before)|stopped before configured final checkpoint", re.I),
    ),
)
