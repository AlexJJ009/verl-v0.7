...
def cleanup(ownership: dict, grace_seconds: float = 10.0) -> dict:
    ...
    if pgid > 0:
        actions.append({"kind": "process_group", **terminate_group(pgid, grace_seconds)})
    for pid in ownership.get("descendant_pids", []):
        if alive(int(pid)):
            try:
                os.kill(int(pid), signal.SIGKILL)
                actions.append({"kind": "descendant", "pid": int(pid), "returncode": 0})
            except OSError as exc:
                actions.append({"kind": "descendant", "pid": int(pid), "returncode": 1, "error": str(exc)})
    for session in ownership.get("tmux_sessions", []):
        actions.append({"kind": "tmux", **command(["tmux", "kill-session", "-t", session])})
    for container in ownership.get("docker_containers", []):
        actions.append({"kind": "docker", **command(["docker", "rm", "-f", container])})
    ...
    command_failures = [
        ...
        and not (
            item["kind"] in {"tmux", "docker"}
            and "not found" in (item.get("stderr", "") + item.get("stdout", "")).lower()
        )
    ]
    ...
    return {
        ...
        "live_run_gpu_pids": live_gpu_pids,
        "cleanup_failures": command_failures,
        "ownership_attribution_proven": attribution_proven,
...
def main() -> int:
    ...
    parser.add_argument("--now-epoch-s", type=float)
    parser.add_argument("--grace-seconds", type=float, default=10)
    args = parser.parse_args()
    ownership = json.loads(args.ownership.read_text())
    state = evaluate(ownership, args.now_epoch_s if args.now_epoch_s is not None else time.time())
    report = {
        "schema_version": 1,
    ...
    if state["timed_out"]:
        report.update(
            {"status": "blocked", "reason": "validation_deadline_exceeded", **cleanup(ownership, args.grace_seconds)}
        )
    elif state["complete"]:
